...
void solve() {
    ...
    bool updated = false;
    ll updated_to;
    unordered_map<ll, ll> indices_changed_after_update;
    while(q--){
        ll t = readl();
        if(t == 1){
            ll i = readl(), x = readl();
            if(updated){
                if(indices_changed_after_update.count(i - 1)) sum = sum - indices_changed_after_update[i - 1] + x;
                else sum = sum - updated_to + x;
                
                indices_changed_after_update[i - 1] = x;
            }
            else{
                sum = sum - a[i - 1] + x;
                a[i - 1] = x;
            ...
            updated = true;
            updated_to = x;
            indices_changed_after_update.clear();
        }
    }
    return;
}
