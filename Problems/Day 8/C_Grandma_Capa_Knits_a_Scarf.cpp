...
ll solve() {
    ...
    // from the both ends, find the first pair that doesnt match
    int l = 0, r = n - 1;
    while(l < r){
        if(s[l] != s[r]){
            possible.pb(s[l]);
    ...
    fe(c, possible){
        ...
        while(l < r){
            ...
            // if the two letters match, move both pointers closer
            if(s[l] == s[r]){
                l++;
                r--;
            }
            // else if the letter is c and the two letters don't match, remove c and increment the counter
            else if(s[l] == c){
                count++;
                l++;
            }
