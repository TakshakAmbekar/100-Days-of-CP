...
ll solve() {
    ll n = readl(), k = readl(), x = readl();
    vl students = readvl(n);
    sort(all(students));

    // now we need to find the gaps between consecutive students.
    // if the gap is bigger than x then we either need to start a new group or invite one or more student in between
    // inviting 1 student can fill up a 2x gap, 2 students can fill up a 3x gap and so on... a gap of length l will require (l - 1) / x students 
    // it is optimal to fill the smallest gaps first

    vl gaps;
    f(i, 0, n - 1){
        ll gap = students[i + 1] - students[i];
        if(gap > x) gaps.pb(students[i + 1] - students[i]);
    }
    ...
    fe(gap, gaps){
        ll students_required = ((gap - 1)/ x);
        if(students_required > k) break;
        k -= students_required;
        groups--;
    }
