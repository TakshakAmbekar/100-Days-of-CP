...
ll solve() {
    ...
    // cf(i, 1, n){
    //     int next_index;
    //     if(i % a[i] == 0) next_index = i + a[i];
    //     else next_index = i + ((a[i] + (a[i] - 2 * (i % a[i]))) % a[i]);
        
    //     if(next_index > n) continue;
    //     for(int j = next_index; j <= n; j += a[i]){
    //         if((ll)a[i]*a[j] == (ll)i + j){
    //             count++;
    //         }
    //     }
    // }
    ...
    f(i, 0, n - 1){
        if(values[i].first > n) break;
        f(j, i + 1, n){
            if((ll)values[i].first * values[j].first > 2LL * n) break;
            if((ll)values[i].first * values[j].first ==  values[i].second + values[j].second) count++;
        }
    }
    return count;
}
