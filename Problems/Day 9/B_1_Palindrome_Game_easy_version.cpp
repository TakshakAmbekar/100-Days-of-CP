...
string solve() {
    ...
    if(n == 1) return s[0] == '0' ? "BOB" : "DRAW";

    // If all are 1's then it is a draw
    int zeros = 0;
    fe(c, s) zeros += (c == '0');
    if(zeros == 0) return "DRAW";
    
    // if there is only one 0, Alice would be forced to flip it
    if(zeros == 1) return "BOB";

    // if the string length is even or length is odd and the middle character is 1, BOB will win because he will keep mirroring the flips till the end and on last turn he will flip 
    // if the string length is odd and the middle character is 0, then Alice will flip it and adopt the same strategy as bob did of flipping the mirror
    return (n % 2 == 0 || s[n/2] == '1') ? "BOB": "ALICE";
}
