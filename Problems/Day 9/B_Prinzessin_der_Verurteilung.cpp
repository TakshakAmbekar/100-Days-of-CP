...
// Hints used : 0
//----------------------------------------------------------------------------------------------------
bool present(string &s, string &pattern){
    int n = s.size(), l = pattern.size();
    if(n < l) return false;
    f(i, 0, n - l + 1){
        bool okay = true;
        f(j, 0, l){
...
string solve() {
    ...
    f(i, 1, 27){
        string pattern = string(1, letters[i]);
        if(!present(s, pattern)) return pattern;
    }

    f(i, 1, 27){
        f(j, 1, 27){
            string pattern = string("") + letters[i] + letters[j];
            if(!present(s, pattern)) return pattern;
        }
    }
    ...
    f(i, 1, 27){
        f(j, 1, 27){
            f(k, 1, 27){
                string pattern = string("") + letters[i] + letters[j] + letters[k];
                if(!present(s, pattern)) return pattern;
            }
