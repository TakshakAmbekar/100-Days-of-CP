...
// Time taken to solve : 1hr 13 mins
// Hints used : 0
//----------------------------------------------------------------------------------------------------
...
ll solve() {
    ...
    // pair up numbers that have remainders that sum up to m
    cf(i, 1, (m - 1) / 2){
        int freq1 = remainders[i], freq2 = remainders[m - i];
        if(freq1 == 0 && freq2 == 0) continue;
        if(abs(freq1 - freq2) <= 1) ans++;
        else{
    ...
    // all multiples of m can be taken in a single array
    if(remainders[0] > 0) ans++;

    // if m is even, then all numbers with remainder m/2 can be taken in a single array
    if(m % 2 == 0 && remainders[m / 2] > 0) ans++;
