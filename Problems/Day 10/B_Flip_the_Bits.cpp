...
// Time taken to solve : 22 mins 
// Hints used : 0
//----------------------------------------------------------------------------------------------------

string solve() {
    int n = readi();
    string a = reads(), b = reads();
    vi numsa(n), numsb(n);
    f(i, 0, n){
        if(a[i] == '0') numsa[i] = -1;
        else numsa[i] = 1;
    ...
    prefix[0] = numsa[0];
    f(i, 1, n)
        prefix[i] = prefix[i - 1] + numsa[i];
        
    int multiplier = 1;
    rf(i, n, 0){
        // if the numbers don't match we need to flip till ith index
        if(numsa[i] * multiplier != numsb[i]){
            // if prefix[i] != 0 then we cannot form
            if(prefix[i] != 0) 
                return "NO";
            else{ 
                multiplier *= -1;
