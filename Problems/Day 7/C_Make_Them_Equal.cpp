...
vl solve() {
    ...
    // 0 operations needed
    if (all_of(all(s), [&](char ch){ return ch == c; })) return {};

    // 1 operation needed if last letter is c
    if(s[n - 1] == c) return {n};
    
    // 1 operation with some number in [2,n] if all multiples of x are c
    for(int x = 2; x <= n; ++x){
        bool ok = true;
        for (int j = x; j <= n; j += x){
            if (s[j-1] != c){ 
