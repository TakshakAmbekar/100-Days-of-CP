...
ll solve() {
    ...
    // a pair will have its & greater than ^ only if the first setbit is the same in both the numbers
    // so we can count the frequency of each setbit, and then pair them groupwise and add up all the answers

    // a pair will have its & equal to ^ only if both the numbers are 0
    // this too will be handled using same approach
    
    vl msb_count(31);
    fe(num, a){
        ll msb = msb_index(num);
        msb_count[msb]++;
    }
