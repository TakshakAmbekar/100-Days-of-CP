...
// Time taken to solve : 3 hours
// Hints used : Editorial
//----------------------------------------------------------------------------------------------------

pair<string, vector<pi>> solve() {
    int n = readi();
    vi a(n + 1);
    cf(i, 1, n) a[i] = readi();

    // check if all gangs are the same
    bool all_same = true;
    cf(i, 2, n) if (a[i] != a[1]) { all_same = false; break; }
    if (all_same) return {"NO", {}};

    vector<pi> routes;
    int root = 1;

    // find an index with a different gang than root
    int other = -1;
    cf(i, 2, n) if (a[i] != a[root]) { other = i; break; }

    // connect root with every district of a different gang
    cf(i, 2, n) {
        if (a[i] != a[root]) routes.pb({root, i});
    }

    // connect 'other' with every remaining district of the same gang as root (except root)
    cf(i, 2, n) {
        if (a[i] == a[root]) routes.pb({other, i});
    }
...
int main() {
    ...
    ll t = readl();
    while(t--) {
        pair<string, vector<pi>> ans = solve();
        print(ans.first, 1);
        if(!ans.second.empty())
            fe(route, ans.second)
                print(route);
