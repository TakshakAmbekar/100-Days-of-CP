...
/*---------Observations---------
    1. rotate the matrix 3 times and check how many unique numbers are there at each cell
*/
...
ll solve() {
    ...
    f(i, 0, n){
        f(j, 0, n){
            ...
            zeros += (grid[i][j] == 0);                          // original
            zeros += (grid[j][n - 1 - i] == 0);                  // rot1
            zeros += (grid[n - 1 - i][n - 1 - j] == 0);          // rot2
            zeros += (grid[n - 1 - j][i] == 0);                  // rot3

            if (zeros == 0 || zeros == 4) continue;
            count += (zeros == 1 || zeros == 3) ? 1 : 2;
        }
    }
